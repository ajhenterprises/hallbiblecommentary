...
export function Header({active=''}:{active?:string}){const site=useSite();return <header className="header"><a href="/" className="brand"><BookOpen size={29}/><span className="brand-name">{site.brandName}</span></a><MobileMenu/><nav aria-label="Main navigation">{site.primaryLinks.map(([n,url])=><a className={n===active?'active':''} href={url} key={url}>{n}</a>)}<NavMenu label="Resources" links={site.resourceLinks}/><NavMenu label="About" links={site.aboutLinks}/></nav><a className="icon-button" href="/search" aria-label="Search"><Search size={20}/></a><a className="button primary header-subscribe" href="/subscribe">Subscribe</a></header>}
