...
export function EntryCards({items}:{items:any[]}){return <div className="cards">{items.map(c=><a href={'/'+(c.kind==='blog'?'articles':c.kind==='sermon'?'sermons':'commentary')+'/'+c.slug} className="card" key={c.id}><span className="eyebrow">{c.kind==='blog'?'ARTICLE':c.kind==='sermon'?'SERMON':'COMMENTARY'}</span><h3>{c.title}</h3><p>{plainText(c.excerpt||c.body).slice(0,160)+(plainText(c.excerpt||c.body).length>160?'…':'')}</p><span className="card-date">{c.publish_date?new Date(c.publish_date).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}):''}</span></a>)}</div>}
